...
module holasui_quest::quest_test {
    ...
    #[test]
    #[expected_failure(abort_code = quest::EJourneyNotCompleted)]
    fun complete_journey_but_some_quests_not_completed() {
        let test = ts::begin(ADMIN);
        ...
        let coin = coin::mint_for_testing<SUI>(
            quest::fee_for_creating_journey(&mut hub),
            ts::ctx(&mut test)
        );
        let journey_id = quest::create_journey(
            &mut hub,
            coin,
            &space_admin_cap,
            &mut space,
    ...
    fun check_soulbound_reward() {
        ...
        let soulbound_reward = ts::take_from_sender<SoulboundReward>(&test);
        print(&soulbound_reward);
        ...
        ts::return_shared(space);
        ts::return_to_address(CREATOR, space_admin_cap);
        ts::return_to_sender(&test, soulbound_reward);
        ts::end(test);
    }


    #[test]
    fun check_nft_reward() {
        let test = ts::begin(ADMIN);
        ...
        let soulbound_reward = ts::take_from_sender<NftReward>(&test);
        print(&soulbound_reward);
